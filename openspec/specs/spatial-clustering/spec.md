# spatial-clustering Specification

## Purpose
Defines the core hierarchical agglomerative clustering algorithm: weighted AABB distance calculation, polygon merging, content ordering after merge, per-page processing isolation, locked cluster support, and the `MaxMergeDistance` safety valve that prevents over-merging on sparse pages.
## Requirements
